...
export async function createArchiveTestData(connection: Connection, num: number): Promise<Archive[]> {
  ...
  for (let index = 0; index < num; index++) {
    ...
    const query = `insert into archives(outer_link,talent_id,video_title,video_thumbnail,open_date) values("${archive.outer_link}","${archive.talent_id}","${archive.video_title}","${archive.video_thumbnail}","${formattedDate}")`;
